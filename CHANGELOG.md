...
* Removed the `EdgeGridV1Signer` class and replaced it with `EdgeGridV2Signer`.
...
* Added automatic redirect handling with request resigning, via the `EdgeGridV2Signer.CreateHttpClient()` factory method.
...
* `EdgeGridV2Signer.CreateHttpClient()`. The factory method that creates an `HttpClient` with automatic redirect handling and resigning.
